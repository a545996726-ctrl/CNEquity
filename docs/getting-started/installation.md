...
| 磁盘 | 日频主干几百 MB 级（全市场日频 2001–2026 约 468 MB）。分钟线默认关闭（全市场 1m 约 8.4 GB/年）。含 staging / revision 的生产湖可以到十几 GB |
...
- 依赖（duckdb / polars / pyarrow 等）均有 `win_amd64` 轮子；若某包退化成从源码编，`cne doctor` 会报出。
...
旧文档里的 `pip install "cnequity[tdx]"` 之类仍然可用，装出来的结果完全一致——pip 会提示一句 `does not provide the extra 'tdx'` 然后照常安装，uv 则不作声。
...
`configs/cnequity.toml`、`data/`、根目录 `logs/` 均已 gitignore，请勿强制加入版本库。
...
3. **孤儿包**：AkShare 已移除（[issue #3](https://github.com/rootSunc/CNEquity/issues/3)），pip / uv 不会卸掉不再依赖的包：
   ...
   `cne doctor --fix` 已删除（只修 mini-racer 冲突）。
